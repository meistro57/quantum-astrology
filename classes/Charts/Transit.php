...
class Transit
{
    ...
    public function getCurrentTransits(?DateTime $transitDate = null): array
    {
        ...
        try {
            ...
            $currentPositionsRaw = $this->swissEph->calculatePlanetaryPositions(
                $transitDate,
                $this->natalChart->getBirthLatitude(),
                $this->natalChart->getBirthLongitude()
            );
            $currentPositions = $this->normalizePositions($currentPositionsRaw);

            $natalPositions = $this->normalizePositions($this->natalChart->getPlanetaryPositions());
    ...
    public function generateTransitChart(?DateTime $transitDate = null): array
    {
        ...
        return [
            ...
            'inner_wheel' => $this->normalizePositions($this->natalChart->getPlanetaryPositions()),
    ...
    /**
     * Normalize planet position payloads into keyed entries with longitude fields.
     *
     * @param array<mixed> $positions
     * @return array<string, array{longitude: float, longitude_speed?: float}>
     */
    private function normalizePositions(array $positions): array
    {
        ...
        foreach ($positions as $key => $row) {
            ...
            $planetName = is_string($key) && !is_int($key)
                ? strtolower(trim($key))
            ...
            $speed = $row['longitude_speed'] ?? $row['speed'] ?? null;
            if (is_numeric($speed)) {
                $normalized[$planetName]['longitude_speed'] = (float)$speed;
            }
        }
