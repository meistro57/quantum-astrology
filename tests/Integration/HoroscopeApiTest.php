...
final class HoroscopeApiTest extends TestCase
{
    ...
    public function testRejectsInvalidPeriod(): void
    {
        ...
        try {
            ...
            $this->assertSame('Invalid horoscope period.', $payload['error'] ?? null);
        } finally {
            @unlink($dbPath);
        }
    }
    ...
    /** @runInSeparateProcess */
    public function testGeneratesHoroscopePreviewAndDownloadUrl(): void
    {
        [$dbPath, $chartId, $ownerId] = $this->bootstrapDbWithChart();
        ...
        try {
            [$status, $output] = $this->runEndpoint($dbPath, [
                ...
                'period' => 'daily',
                'for_date' => '2026-03-03',
                'areas' => ['love', 'career', 'health'],
                'focus' => 'practical decisions',
                'format' => 'json',
                'provider' => 'openai',
                'model' => 'default',
            ], [
            ...
            $this->assertStringContainsString('#', (string)($payload['markdown'] ?? ''));
            $this->assertStringContainsString('horoscope.php', (string)($payload['download_url'] ?? ''));
            $this->assertStringContainsString('format=download', (string)($payload['download_url'] ?? ''));
            $this->assertStringContainsString('Overview', (string)($payload['html_preview'] ?? ''));
        } finally {
            @unlink($dbPath);
        }
    }
    ...
    {
        $dbPath = tempnam(sys_get_temp_dir(), 'qa_horoscope_');
        if ($dbPath === false) {
    ...
    private function bootstrapDbWithChart(): array
    {
        ...
        $user = User::create([
            'username' => "horoscope_user_{$uniq}",
            'email' => "horoscope_user_{$uniq}@example.com",
...
$_SERVER = array_merge($_SERVER, [
    'REQUEST_METHOD' => 'POST',
    'REQUEST_URI' => '/api/reports/horoscope.php',
...
PHP;
        ...
        $code = strtr($bootstrap, [
            ...
            '%ENTRYPOINT_PATH%' => var_export($this->projectPath('api/reports/horoscope.php'), true),
        ]);
